 ...
 *
 * - SM4 optimization for ARMv8 by NEON and AESE instruction
 */
		   ...
		   const uint8_t *input, uint8_t *output)
{
	...
	if (ctx->mode == SM4_ENCRYPT) {
		crypto_accel_sm4_xts_enc(output, input, ctx->sk, ctx_ek->sk, len, iv);
	} else {
		crypto_accel_sm4_xts_dec(output, input, ctx->sk, ctx_ek->sk, len, iv);
	}
}
